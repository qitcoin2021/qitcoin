...
//! Default for -keypool
static const unsigned int DEFAULT_KEYPOOL_SIZE = 1;
//! -paytxfee default
...
public:
    ...
    /**
     * Whether this output frozen in bind plotter or point
     */
    bool fFrozen;

    COutput(const CWalletTx *txIn, int iIn, int nDepthIn, bool fSpendableIn, bool fSolvableIn, bool fSafeIn, bool fFrozenIn, bool use_max_sig_in = false)
    {
        tx = txIn; i = iIn; nDepth = nDepthIn; fSpendable = fSpendableIn; fSolvable = fSolvableIn; fSafe = fSafeIn; fFrozen = fFrozenIn; nInputBytes = -1; use_max_sig = use_max_sig_in;
...
public:
     ...
     */
    void AvailableCoins(interfaces::Chain::Lock& locked_chain, std::vector<COutput>& vCoins, bool fOnlySafe = true, bool fIncludeFreeze = false, const CCoinControl* coinControl = nullptr, const CAmount& nMinimumAmount = 1, const CAmount& nMaximumAmount = MAX_MONEY, const CAmount& nMinimumSumAmount = MAX_MONEY, const uint64_t nMaximumCount = 0) const EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    ...
    std::vector<CTxDestination> GetExternalAddresses(int64_t fromIndex, int64_t toIndex, int64_t &total) const;
    ...
    boost::signals2::signal<void (CWallet *wallet)> NotifyPrimaryAddressChanged;
