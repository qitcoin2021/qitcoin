    ...
    transactionView(nullptr), abandonAction(nullptr), removeAction(nullptr), bumpFeeAction(nullptr), unfreezeAction(nullptr), columnResizingFixer(nullptr)
{
    ...
    typeWidget->addItem(tr("Received with"), TransactionFilterProxy::TYPE(TransactionRecord::RecvWithAddress) |
        ...
        TransactionFilterProxy::TYPE(TransactionRecord::PointReceived) |
    ...
    typeWidget->addItem(tr("Sent to"), TransactionFilterProxy::TYPE(TransactionRecord::SendToAddress) |
        ...
        TransactionFilterProxy::TYPE(TransactionRecord::PointSent) |
        TransactionFilterProxy::TYPE(TransactionRecord::SelfPoint));
    ...
    typeWidget->addItem(tr("Point sent"), TransactionFilterProxy::TYPE(TransactionRecord::PointSent) |
        TransactionFilterProxy::TYPE(TransactionRecord::SelfPoint));
...
void TransactionView::unfreezeTx()
{
    if(!transactionView || !transactionView->selectionModel())
        return;
    QModelIndexList selection = transactionView->selectionModel()->selectedRows(0);
    ...
    // Unfreeze transaction over the walletModel
    if (model->unfreezeTransaction(hash)) {
        // Update the table
        model->getTransactionTableModel()->updateTransaction(hashQStr, CT_UPDATED, false);
    }
}
