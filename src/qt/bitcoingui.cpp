...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Qitcoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and qitcoin: URIs)"));
    ...
    aboutAction = new QAction(tr("&About %1").arg(BitcoinGUI::tr(PACKAGE_NAME)), this);
    aboutAction->setStatusTip(tr("Show information about %1").arg(BitcoinGUI::tr(PACKAGE_NAME)));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for %1").arg(BitcoinGUI::tr(PACKAGE_NAME)));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Qitcoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Qitcoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a qitcoin: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Qitcoin command-line options").arg(BitcoinGUI::tr(PACKAGE_NAME)));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    if (QSystemTrayIcon::isSystemTrayAvailable()) {
        ...
        QString toolTip = tr("%1 client").arg(BitcoinGUI::tr(PACKAGE_NAME)) + " " + m_network_style->getTitleAddText();
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Qitcoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitcoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret)
{
    ...
    QString strTitle{BitcoinGUI::tr(PACKAGE_NAME)};
...
{
    QString window_title = BitcoinGUI::tr(PACKAGE_NAME);
#ifdef ENABLE_WALLET
    if (walletFrame) {
