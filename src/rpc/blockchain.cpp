...
static UniValue getchaintips(const JSONRPCRequest& request)
{
            RPCHelpMan{"getchaintips",
                ...
                {
                    {"verbose", RPCArg::Type::BOOL, /* default=false */ "false", "True will output details."},
                },
                RPCResult{
            "[\n"
            "  {\n"
    ...
    for (const CBlockIndex* block : setTips)
    {
        ...
        if (!fVerbose && branchLen > 0 && ::ChainActive().Tip()->nTime > block->nTime + 24 * 3600)
            continue;
...
UniValue scantxoutset(const JSONRPCRequest& request)
{
    ...
    if (request.params[0].get_str() == "status") {
        ...
        g_scan_progress = 0;
        int64_t count = 0;
        CCoinsViewCursorRef pcursor;
        CBlockIndex* tip;
        {
            LOCK(cs_main);
            ::ChainstateActive().ForceFlushStateToDisk();
            pcursor = ::ChainstateActive().CoinsDB().Cursor();
            assert(pcursor);
            tip = ::ChainActive().Tip();
            assert(tip);
        }
