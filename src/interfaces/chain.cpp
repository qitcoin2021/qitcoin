...
public:
        ...
        if (bindHeight > consensusParams.nBindPlotterCheckHeight) {
            ...
            if (!lastBindInfo.outpoint.IsNull()) {
                ...
                if (bindHeight < limitBindHeight) {
                    return std::make_pair(Consensus::GetBindPlotterPunishmentAmount(bindHeight, consensusParams), limitBindHeight);
                } else {
                    return std::make_pair(0, limitBindHeight);
                }
            }
        }
    ...
    bool haveCoin(const COutPoint &outpoint) override
    {
        LOCK(::cs_main);
        return ::ChainstateActive().CoinsTip().HaveCoin(outpoint);
    }

    const Coin& accessCoin(const COutPoint &outpoint) override
    {
        LOCK(::cs_main);
